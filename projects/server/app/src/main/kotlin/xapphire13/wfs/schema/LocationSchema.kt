...
fun SchemaBuilder.locationSchema() {
  ...
  property<RemoteItem.RemoteFolder>("rootFolder") { 
    resolver {location -> 
      val path = Path.of(location.rootPath)
      val attr = Files.readAttributes(path, BasicFileAttributes::class.java)
      
      RemoteItem.RemoteFolder("/", attr.creationTime().toInstant(), attr.lastModifiedTime().toInstant())
    }
   }
 }
  ...
  query("locations") {
    ...
    resolver { -> listOf(RemoteLocation("1", "My Drive", "/home")) }
  }
  ...
  query("location") {
    ...
    resolver {id: String -> 
      if (id !== "1") throw NotFoundException("Can't find location with id: $id")
